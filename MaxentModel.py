...
"""
 @description:
        ...
        根据给定语料描述预测outdoor或者indoor
        每个给定的语料中的词作为特征函数feature,  x为语料中的词,y为label
        最后给定一个词,预测indoor还是outdoor.
        f(x, y): x:word, y:outdoor or indoor
        P(y|x)即给定一个词,预测是indoor还是outdoor
        从给定的每一行中抽取feature的统计信息.


        程序首先是构建特征函数.  本例是label和词
        其他可以定义词性和前后出现的词性,当特征达到一定的阈值可以将其作为feature,比如概率超过0.8
        为什么不像掷骰子一样,设置简单的判断条件,
            1\特征函数过多
            2\多个特征函数作用于一个label
    ...
    def load_data(self, file):
        for line in open(file):
            ...
            for f in set(fields[1:]):
                # (label,f) tuple is feature, such as (sunny, outdoor)
...
if __name__ == '__main__':
    model = MaxEnt()
    model.load_data('maxent.txt')
    model.train()
    rs = model.predict('Cloudy')
    for p, l in rs:
        print p
        print l
